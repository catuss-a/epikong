...
** Last update Sun Mar 27 23:08:06 2011 axel catusse
*/
...
void			display_runright_robot(t_epikong *ekg, t_char *monster)
{
  ...
  sprite.w = monster->animation.runright[(int)monster->idx_anim].w;
  sprite.h = monster->animation.runright[(int)monster->idx_anim].h;
  sprite.x = monster->animation.runright[(int)monster->idx_anim].pos.x;
  sprite.y = monster->animation.runright[(int)monster->idx_anim].pos.y;
