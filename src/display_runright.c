...
** Last update Sun Mar 27 23:07:36 2011 axel catusse
*/
...
void			display_runright(t_epikong *ekg)
{
  ...
  sprite.w = ekg->hero_l->animation.runright[(int)i].w;
  sprite.h = ekg->hero_l->animation.runright[(int)i].h;
  sprite.x = ekg->hero_l->animation.runright[(int)i].pos.x;
  sprite.y = ekg->hero_l->animation.runright[(int)i].pos.y;
  i += 0.05;
